USE LANCHONETE
GO
CREATE FUNCTION RetornaResumoAnoMes(@Ano INT, @Mes INT)
RETURNS TABLE AS 
RETURN
(
SELECT @Mes AS MES,  
...
ISNULL(((SELECT SUM(VALOR) AS Entradas FROM tbCaixa WHERE TipoEvento = 'Entrada' AND CategoriaLancamento NOT IN(3, 4) AND MONTH(DataEvento)=@Mes AND YEAR(DataEvento)=@Ano) + (SELECT SUM(VALOR) AS Entradas FROM tbCaixa WHERE TipoEvento = 'Saida' AND MONTH(DataEvento)=@Mes AND YEAR(DataEvento)=@Ano)),0) AS Lucro  
)